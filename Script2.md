...
[Vo2: Versuchsdesigns:](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#vo2-versuchsdesigns)
  ...
  - [18. Interne und externe Validität, Zusammenhang mit Feld- bzw- Laborstudien.](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#18-interne-und-externe-validität-zusammenhang-mit-feld--bzw--laborstudien)
...
[Vo.3 Datenerfassung](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#vo3-datenerfassung)
  ...
  - [40. Ratingskalen](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#40-ratingskalen)
...
[Vo.4 Kausalität](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#vo4-kausalität)
  ...
  - [58. Postnormale Wissenschaft = vertritt selbst werte](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#58-postnormale-wissenschaft)
...
[Vo.6 Zeitreihendesigns](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#vo6-zeitreihendesigns)
...
[Vo.8 Stichproben]()
  ...
  - [118. Intraklassenkorrelation](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script2.md#118-intraklassenkorrelation)
...
### 64. Designs ohne Kontrollgruppe (Ein-Gruppen-Post-Test, Ein-Gruppen-pretest-posttest, removed treatment)
...
```
nicht experimentell
Überprüft wird ob bei ProbantInnen mit ADHS das trinken einer tasse kaffe vor dem schlafen zu qulitativ besserem schlaf führt.

Das Problem ist das wir nicht wissen wie die schlafqualität ohne die tasse kaffee ausgesehen hätte und können damit auch nicht einwandfrei sagen ob die vermeindlich bessere schlafqualität an der tasse gelegen hat. Es kann also nicht geschlossen werden das die tasse kaffee die schlafqualität verbessert.
```
...
```
nicht experimentell
Die schlafqualität von probandInnen wird gemessen. anschließend wird Überprüft ob bei ProbantInnen mit ADHS das trinken einer tasse kaffe vor dem schlafen zu qulitativ besserem schlaf führt.

selbst eine qualitative verbesserung der schlafqualität wird nicht einwandfreie rückschlüsse darauf schließen lassen, dass die vermeindlich bessere schlafqualität wirklich an der tasse kaffe liegt, und nicht an anderen einflüssen, vllt durch die regelmäßigere konrtrolle des schlafes.
```

- posttest mit zwei verschiedenen Gruppen

```
eine gruppe adhsler trinkt eine tasse kaffe vor dem schlafen, die andere nicht.

experimentell, lässt rückschlüsse zu, da der pretest fehlt besteht aber die gefahr systematische unterschiede zwischen den gruppen die a priori bestehen nicht festzustellen.
```

- Versuchsplan mit Prä-Post-Test und Kontrollgruppe:

```
pre test befragung nach schlafqualität. eine gruppe adhsler trinkt eine tasse kaffe vor dem schlafen, die andere nicht.

experimentell, randomisierung kann aufrechterhalten werden, jedoch könnte die messung als solche und das befragen nach der schlafqualität dazu führen das diese sich verbessert. wäre dem so nicht experimentell.
```
...
ein gruppen pretest posttest -> nicht experimentell
...
-posttest mit zwei verschiedenen gruppen. quasi-experimentell
...
ein gruppen post test- nicht experimentell
...
```
Multiple Kontrollgruppen: verschiedene Kontrollgruppen, um verschiedene alternative Erklärungen auszuschließen
  • Beispiel: Teenagerinnen mit Abtreibung vs. Frauen welche Kind bekommen + Frauen welche abtreiben wollten aber dann gar nicht schwanger waren (Zabin et al. 1989). Nach Abtreibung mehr Bildung als BEIDE anderen Gruppen (auch die ohne nachfolgende Kinderbetreuung), also BildungsVERLUST durch Abtreibung schwer zu argumentieren

Präzisierung der Vorhersagen: (= spezifische Interaktion mit Versuchsbedingungen)
  • Beispiel: Kinder mit älteren Geschwistern bei selber Lehrperson haben bessere Leistungen (durch Lehrer*innenerwartung; Seaver 1973). Design: Vergleiche ältere und jüngere Geschwister, geteilt nach solchen mit gleichen Lehrer*innen und solchen mit verschiedenen.
```
...
+- 1.96*(0.5/(wurzel800!)) = 0.035 = +- 3.5%

theoretisch noch plus mittelwert +- oben.
...
```
5^2 = 25 um um faktor 5 kleiner zu werden 5/1= 5
```
...
2*(0.5/wurzle900) = 0.0333333 ergo +-3.33% das ergebnis kann sich noch ändern.
bei 1000 _> 0.0316 = +-3.16% auch hier kann sich das ergebnis noch knapp ändern.
...
```
In mehreren fMRT Labors werden im anschluss an eine untersuchung Fragebögen an teilnehmer ausgegeben die Anxiety tendenzen im alltag/als trait erfragen. durchschnittliche klumpengröße = 25(also zum beispiel 10 labors mit )

die probantInnen sind weitestgehend anonym, daher das design als klumpenstichprobe.

es ist möglicherweise davon auszugehen, das sich ProbandInnen die sich freiwillig für eine fMRT untersuchung melden generell weniger anxiety verhalten zeigen. -> daher in hinblick auf anxiety geringe interklassenunterschiede zu befürchten -> geschätzte interklassenkorrelation 0.24.

Designeffekt

1 + 0.24 * (25 - 1) = 6,76 -> effektive stichprobengröße um das 6,76 fache veringert.

kaum gruppenunterschiede hinsichtlich anxiety.
```
...
```
Wenn es fehlende Werte gab
MCAR -> nicht so schlimm
MAR -> wie wurde korrigiert?
notMAR -> supergau
```
