...
[Markdown - Link]([#-Link1](https://github.com/Octo-Spork/Octo-Spork.github.io/blob/main/Script.md#vo2----arbeitsmittel-psychologischen-diagnostizierens))
...
###Vo_1: ### VO_1: Ethische und rechtliche Rahmenbedingungen psychologischen Diagnostizierens:
...
Aus Dorsch - Lexikon der psychologie
...
KABC-II aud wechsler

beide pragmatische intelligenztheoretische positon: gemessen wird, was zu
messen geht" -> ist testbar
...
pragmatischer Standpunkt -> viele Fähigkeiten die für intelligentes
...
Differenzielle Psychologie -> diagbostik
...
> Rosenthal-Effekt -> Lehrer bringen zwei Versuchobjekte (Ratten,
...
### Pychologisches Diagnostizieren
	...
	- welche gegebenheiten erwünschtes (erleben und verhalten im gegebenen fall verhindern

		oder

		unerwünschtes erleben und verhalten verursachen
		...
		!Westhoff und Kluck 2003 - Verhaltensgleichung
		- Bedingungsvariablen zur Erklärung, prognose und veränderung
		...
		differenzielle psych trägt nichts zur anamnese bzw exploration bei -> nur
		psychotherapeutische Schulen
		)

		Auswahl der einzusetzenden Verfahren (

			auswahl anhand von gütekriterien_
			Situationsunabhängigkeit <-> item-stichprobe


			ad situationsunabhängigkeit -> psych diagnostik -> art und weise der feststellung von unterschieden zwischen einer bestimmten konkreten person und anderen, auch abstrakten zB ideal typischen personen

			-> 1) Je Situation kann ein anderes Verhalten gezeigt werden
			-> 2) Psychologisched Diagnostizieren setzt Compliance
			(Mitmachbereitschaft) voraus!

			-> Compliance kann schon bei der untersuchung selbst problematisch sein wenn sie fremdbeschtimmt beauftragt wurde.


			ad Item-Stichprobe -> Informationsstichprobe indem systematisch erstellte
			Fragen interessierende Verhaltensweisen oder psychische Vorgänge auslösen
			Ziel: frageliche Merkmalsausprägung zu bestimmen.

			Wie bei statistik kann stichprobe zufällig nicht repräsentativ sein
			fragen müssen repräsentativ sein -> aufgabe testkonstrukteur
			zu wenige Fragen werden niemals reichen um den Stichprobenfehler möglichst
			klein zu halten

			wir vermuten das mit größerer anzahl von fragen der fehler immer kleiner wird

			-> Gütekriterien Reliabilität(messgenauigkeit) ökonomie (wirtschaftlichkeit)

			vs zu viele items

			-> gütekriterium Zumutbarkeit)
...
Auswahl der einzusetzenden Verfahren (
    ...
    Prozess-Strategien:
	    ...
	    Anforduerungsanalyse (der weg zum anforderungsprofil) (

	        - Arbeitsplatzanalyse

		    -> beschreibung der tätigkeit, ausführungs und
		    rahmenbedungungen unter aspekt der psychischen struktur
		    und prozesse
	...
	- Untersuchungsstrategien (traditionell oder mittels self assessment (

	    - Self Assessment: fachpsychologisch unkontroliert
	    - diagnostikbegleitende intervention (förderung) abprüfen von
	    wisseninhalten mit rückmeldung welche themenbereiche noch vertiefn
	)
	...
	- Entscheidungsstrategien (
	    ...
	    - 2) konjunktive -> sind jede einzelne Anforderung unabdingbar und nicht kompensierbar
		-> multiples minimum-Kriterium
...
# Vo 6 Vertiefung/ Ergänzung: Gütekriterium Zumutbarkeit {

    - Zumutbarkeit bedeutet zeitlich, psychisch (energetisch motivational und emotional) sowie körperlich schonen
	-> aus anwendung resultierenden nutzen
    ...
    - multiple-choice-format zumutbarkeit (
	-> Glück
    ...
    - Projektive Verfahren zumutbarkeit (
	->  
	...
	zurück zu validität (
		...
		-> Inhaltliche Gültigkeit (test erfasst definitsionsgemäß interessierende merkmal)
			...
			1) Experten-Rating (
			    ...
			    spannung Tkonsrukteur vs diagnostiker
				-> testkonstrukteur behauptet experten haben geratet
		...
		-> kriteriumsvalidiät (
			...
			- 0,8 bedeutet 0,64% erklärter varianz bzw variabilität
			-> wird quadriert!!!
...
### VO_8 Grundlagen zur Theorie und Praxis von Leistungstests

## Gestaltungsweisen ()


### 1) -> freies Antwortformat vs Muliple-Choice-Format (
			-> mc ist verrechnungssicher
			-> MC ist verrechnungssicher -> keine mehrdeutigkeit
			-> aber weniger aufschlussreich als freiAntwort
				-> weil keine gelegenheitsbeobachtungen
...
### Vo 9 Vertiefung/ergänzung: Reliablilität(genauigkeit) & ökonomie (wirtschaftlichkeit)
...
### klassische testtheorie (
			...
			Messgenauigkeit bei klassischer testtheorie (
				...
				-> reliabilitätkoeffizienten nicht die absolute differenz zu 1 sondern differenz des quadrats des reliabilitätkoeeffizienten von 1
					-> quadrat der variabilität/varianz!
				-> ergo 0,8 hat 36 messfehler

				Axiome der klassischen Testtheorie (
					Standartmessfehler herleitung:
					-> FORMEL miu
					->
				)
...
### - { Prinzip, Vorteile,  Strategien (->tailored, ->branched) } -
...
### Strategien_adaptivesTesten (tailored-testing, branched-testing)

		# -> itemparameter sind brereits bekannt, kalibrierungsstudien
			-> schwierigkeit der aufgabe muss feststehen

		tailored-testing(StartDurschnitt->ExtremUm, generellRasch, 60-70Items_davon15hinreichendgena_3-4wenigerAlsBranched) {
		...
		branched-testing(NichtSoGutWieTailored, Strategie(Itemgruppenblocks), doppelteAufgabenBeiKonventionellUngenauerALsBranched ) {

			-> nicht ganz optimal gegenüber tailored

			Strategie (Itemgruppen) {
				Itemgruppen verweisen festverzweigt:
...
RechtssicherheitPsychologischerGutachten (Gütekriterien(Eichung, Skalierung)) {
	...
	Sorgfalt_bei_Auswahl_der_Verfahren_zur_Messung () {
		...
		> Skalierung (Anspruchsniveau, RaschModell) {
			...
			-> Skalierung: Verrechnungsvorschriften relutierende Testwerte bilden die empirischen Verhaltensrelationen adäquat ab.
...
GüteKriterium:Unverfälschbarkeit () {
		...
		- Techniken der Fragebogengestalltung (Statement, Adjektiva) {
		...
		Persönlichkeitstheorien und konzepte von Persönlichkeitsfragebogen () {
			...
			-> geht fast nur aus emperie abgeleitete faktorenanalytisch begründete fragebogenbatterien nach katell
				-> bspw big 5
		...
		ist es gut argumentierbar of es argumentierbar plausibel ist
		das der klient von ihrer verfälschbarkeit keinen gebrauch macht
...
###### Bsp von voher, zahlen in rechnerische Beziehung mit ziel (siebenhandl):
...
###### formale besonderheitung der informationsverarbeitungen
...
### Ansätze der experimentalpsychologischen Verhaltensdiagnostik zur erfassung kognitiver Stile
...
### Ansätzer der experimentalpsychologischen Verhaltensdiagnostik zur Erfassung spezieller Konsturkte
